    ...
    class Project
    {
        ...
        public Project(string _mainPhoto, string _projectPath, int pieces)
        {
            try
            {
                ...
                squareImagePath = Path.Combine(projectPath, "SquarePhoto.jpeg");

                using (Bitmap originalImg = LoadBitmapFixedOrientation(_mainPhoto))
                using (Image squareImg = MakeSquarePhoto(originalImg))
                using (Image resizedImg = resizeImage(squareImg, pieces))
                using (Bitmap finalBitmap = new Bitmap(resizedImg))
                {
                    if (File.Exists(squareImagePath))
                    {
                        File.Delete(squareImagePath);
                    }

                    finalBitmap.Save(squareImagePath, ImageFormat.Jpeg);
                }
            }
            catch (Exception ex)
            {
        ...
        private static Bitmap LoadBitmapFixedOrientation(string path)
        {
            const int OrientationId = 0x0112;

            Bitmap bitmap = new Bitmap(path);

            if (bitmap.PropertyIdList.Contains(OrientationId))
            {
                var prop = bitmap.GetPropertyItem(OrientationId);
                ...
                try
                {
                    bitmap.RemovePropertyItem(OrientationId);
                }
                catch
                {
                }
